...
#To support craft dataset
WSJ_POS_MAPPING['HYPH'] = len(WSJ_POS_MAPPING)
WSJ_POS_MAPPING['AFX'] = len(WSJ_POS_MAPPING)
WSJ_POS_MAPPING['XX'] = len(WSJ_POS_MAPPING)
WSJ_POS_MAPPING['VERB'] = len(WSJ_POS_MAPPING)
WSJ_POS_MAPPING['ADD'] = len(WSJ_POS_MAPPING)
WSJ_POS_MAPPING['NFP'] = len(WSJ_POS_MAPPING)
...
ONTO_MAPPING = {"CARDINAL": 0, "DATE": 1, "EVENT": 2, "FAC": 3, "GPE": 4, "LANGUAGE": 5, "LAW": 6,
    "LOC": 7, "MONEY": 8, "NORP": 9, "ORDINAL": 10, "ORG": 11, "PERCENT": 12, "PERSON": 13,
    "PRODUCT": 14, "QUANTITY": 15, "TIME": 16, "WORK_OF_ART": 17, "-NONE-": 18}
INVERSE_ONTO_MAPPING = {v: k for k, v in ONTO_MAPPING.items()}

ONTO_BIO_MAPPING = {}
for k in ONTO_MAPPING.keys():
    if k == '-NONE-':
        continue
    ONTO_BIO_MAPPING['B-{}'.format(k)] = len(ONTO_BIO_MAPPING)
    ONTO_BIO_MAPPING['I-{}'.format(k)] = len(ONTO_BIO_MAPPING)
ONTO_BIO_MAPPING['O'] = len(ONTO_BIO_MAPPING)
INVERSE_ONTO_BIO_MAPPING = {v: k for k, v in ONTO_BIO_MAPPING.items()}
...
def escape_text(tokens):
    def helper(x):
        x = x.replace('(', '-LRB-').replace(')', '-RRB-')
        return x
    return [helper(x) for x in tokens]

def fake_raw_parse(tokens):
    def helper(tokens):
        if len(tokens) == 1:
            return '(DT {})'.format(tokens[0])
        return '(S {} {})'.format(helper(tokens[:1]), helper(tokens[1:]))
    tokens = escape_text(tokens)
    return '(ROOT {})'.format(helper(tokens))

def get_binary_tree(nltk_tree):
    def helper(tr):
        ...
        if len(tr) == 1:
            return helper(tr[0])
        assert len(tr) == 2
        return (helper(tr[0]), helper(tr[1]))
    nltk_tree_ = copy.copy(nltk_tree)
    nltk_tree_.chomsky_normal_form()
    tr = helper(nltk_tree_)
    return tr
...
class WSJEMNLPReader(object):
    def __init__(self, options=None, lowercase=True, filter_length=0, for_nopunct=False):
        self.lowercase = lowercase
        self.filter_length = filter_length
        self.for_nopunct = for_nopunct
        self.logger = get_logger()
        self.require_output_vocab = options.require_output_vocab
        if self.require_output_vocab:
            self.output_vocab = self.read_vocab(options.output_vocab)
            ...
            for span, label_lst in d:
                ...
                if len(label_lst) > 1:
                    ...
                    if key not in WSJ_CONSTITUENCY_MAPPING:
                        new_key = key[-1]
                        warn['trunc_{}_{}'.format(key, new_key)] += 1
    ...
    def read(self, path):
        ...
        with open(path) as f:
            for line in tqdm(f):
                ...
                if 'raw_parse' not in data:
                    data['raw_parse'] = fake_raw_parse(tokens)
                nltk_tree = nltk.Tree.fromstring(data['raw_parse'])
                if 'binary_tree' not in data:
                    data['binary_tree'] = get_binary_tree(nltk_tree)
                ...
                if self.require_output_vocab:
                    if not any([w in self.output_vocab for w in tokens]):
                        continue
        ...
        # def custom_collate(batch_iterator):
        ...
        # metadata['custom_collate'] = custom_collate
...
class PTBConstraintReader(object):
    def __init__(self, options=None, lowercase=True, filter_length=0, for_nopunct=False):
        self.lowercase = lowercase
        self.filter_length = filter_length
        self.for_nopunct = for_nopunct
        self.require_ner = options.require_ner
        self.logger = get_logger()
        self.require_output_vocab = options.require_output_vocab
        if self.require_output_vocab:
            self.output_vocab = self.read_vocab(options.output_vocab)
    ...
    def read(self, path):
        ...
        with open(path) as f:
            for line in tqdm(f):
                ...
                if 'raw_parse' not in data:
                    data['raw_parse'] = fake_raw_parse(tokens)
                raw_parse = data['raw_parse']
                nltk_tree = nltk.Tree.fromstring(data['raw_parse'])
                if 'binary_tree' not in data:
                    data['binary_tree'] = get_binary_tree(nltk_tree)
                binary_tree = data['binary_tree']

                if 'ner_span' not in data:
                    if 'ner_spans' in data:
                        data['ner_span'] = data['ner_spans']
                    elif 'ner_label' in data:
                        data['ner_span'] = data['ner_label']
                if 'ner_label' not in data:
                    if 'ner_span' in data:
                        data['ner_label'] = data['ner_span']
                ner_labels = data.get('ner_label', None)

                if not 'additional_constraint' in data:
                    if 'ner_label' in data:
                        constraints = data['ner_label']
                    elif 'ner_span' in data:
                        constraints = data['ner_span']
                else:
                    constraints = data['additional_constraint']
                ...
                if self.require_ner:
                    if len([None for pos, size, label in constraints if size > 1]) == 0:
                        continue
                ...
                extra.setdefault('example_ids', []).append(example_id)
                extra.setdefault('constituency_tags', []).append(constituency_tags)
                extra.setdefault('constraints', []).append(constraints)
                extra.setdefault('ner_labels', []).append(ner_labels)
                extra.setdefault('pos_tags', []).append(pos_tags)
                extra.setdefault('binary_tree', []).append(binary_tree)
                extra.setdefault('raw_parse', []).append(raw_parse)

                for k in ['labeled_spans', 'overlaps_with_onto']:
                    if k in data:
                        extra.setdefault(k, []).append(data[k])

        print('total number of instance: ',len(sentences))
        constituency_tags = [add_null_spans(x, tr) for x, tr in zip(self.indexify_constituency_tags(extra['constituency_tags']), extra['binary_tree'])]
        pos_tags = self.indexify_pos_tags(extra['pos_tags'])
