...
class ConstrainedLoss(nn.Module):
    ...
    def initial_chart(self, constraints=None):
        ...
        indicator = torch.full((batch_size,ncells,1), 0, dtype=dtype, device=device)
        ...
        for idx, spans in enumerate(constraints):
            for span in spans:
                ...
                indicator[idx,pos] = 1.0
        return chart, indicator

    def batched_cky(self, s, chart, indicator=None):
        batch_size = self.batch_size
        length = self.length
        device = self.device
        size = self.size
        ...
        for level in range(1, length):
            ...
            if not indicator is None:
                li, ri = get_inside_states(batch_info, indicator, self.index, 1)
                pi = li.view(B,L,N)+ri.view(B,L,N) #B, L, N
                # print('pi',pi.shape)
                max_arg = torch.max(ps,2)[1]
                # print('atgmax',max_arg.shape)
                # print('gather shape',torch.gather(pi,2,max_arg).shape)
                # print('indicator ',indicator[:,offset:offset+L].shape)
                indicator[:,offset:offset+L]+= torch.gather(pi,2,max_arg)
        tree_scores = chart[:,-1]
        if not indicator is None:
            coverage = indicator[:,-1]
            tree_scores -= coverage*1000.0
        return tree_scores
    ...
    def forward(self, score_components, constraints=None):
        constrained_chart, indicator = self.initial_chart(constraints)
        ...
        constr_s = self.batched_cky(s, constrained_chart, indicator=indicator)
...
class TreeStructureV2(nn.Module):
    name = 'tree_structure_v2'
...
structure_v2_class = TreeStructureV2
