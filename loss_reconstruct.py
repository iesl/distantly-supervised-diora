...
class ReconstructFixedVocab(nn.Module):
    ...
    def __init__(self, embeddings, input_size, size, margin=1, path='./resource/ptb_top_10k.txt',
                 word2idx=None, cuda=False, load=True, skip=False, weight=1):
        super().__init__()
        self.load = load
        self.path = path
        self.margin = margin
    ...
    def forward(self, sentences, diora, info, embed=None):
        ...
        loss = self.reconstruct(sentences, cell, self.embeddings, self.mat)
...
reconstruct_class = ReconstructFixedVocab
