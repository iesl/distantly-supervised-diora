...
def get_outside_index(length, level, offset_cache=None, device=None):
    if offset_cache is None:
        offset_cache = get_offset_cache(length)
    index = OutsideIndex()
    pairs = index.get_all_pairs(level, length)
...
def get_topk_outside_index(length, level, K, offset_cache=None, device=None):
    if offset_cache is None:
        offset_cache = get_offset_cache(length)
...
def get_outside_encoded_index(length, offset_cache=None, device=None):
    if offset_cache is None:
        offset_cache = get_offset_cache(length)
    index = OutsideIndex()
...
def get_outside_target(length, level, offset_cache=None, device=None):
    if offset_cache is None:
        offset_cache = get_offset_cache(length)
