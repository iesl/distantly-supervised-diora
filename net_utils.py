...
def build_chart(batch_size, length, size, dtype=None, device=None):
...
class Index(object):
    def __init__(self, device=None, enable_caching=True):
        self.device = device
        self.cache = {}
    ...
    def get_inside_index(self, length, level):
        ...
        def func():
            return get_inside_index(length, level,
                self.get_offset(length), device=self.device)
        return self.cached_lookup(func, name, key)
