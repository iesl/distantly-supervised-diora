...
from embeddings import EmbeddingsReader
from preprocessing import indexify, indexify_etype
from preprocessing import synthesize_training_data
from pretrained_embeddings import build_text_vocab
...
class ReaderManager(object):
    ...
    def run(self, options, text_path, embeddings_path):
        ...
        word2idx = build_text_vocab(sentences, vocab_path=options.init_vocab_path)
        ...
        logger.info('Converting tokens to indexes.')
        sentences = indexify(sentences, word2idx)
    ...
    def initialize(self, options, text_path=None, embeddings_path=None, filter_length=0, data_type=None):
        if data_type in ('txt', 'txt_id'):
            reader = PlainTextReader(lowercase=options.lowercase, filter_length=filter_length,
                                     include_id=data_type == 'txt_id')
        elif data_type == 'wsj_emnlp':
            reader = WSJEMNLPReader(options=options, lowercase=options.lowercase, filter_length=filter_length)

        elif data_type == 'ptb_constraint':
            reader = PTBConstraintReader(options=options, lowercase=options.lowercase, filter_length=filter_length)
